dashboard "compute_instance_detail" {
  ...
  with "compute_instance_from_compute_instance_groups" {
    query = query.compute_instance_from_compute_instance_groups
    args  = [self.input.instance_id.value]
  }

  with "compute_instance_to_compute_disks" {
    query = query.compute_instance_to_compute_disks
    args  = [self.input.instance_id.value]
  }

  with "compute_instance_to_compute_firewalls" {
    query = query.compute_instance_to_compute_firewalls
    args  = [self.input.instance_id.value]
  }

  with "compute_instance_to_compute_networks" {
    query = query.compute_instance_to_compute_networks
    args  = [self.input.instance_id.value]
  }

  with "compute_instance_to_compute_subnets" {
    query = query.compute_instance_to_compute_subnets
    args  = [self.input.instance_id.value]
  }

  with "compute_instance_to_iam_service_accounts" {
    query = query.compute_instance_to_iam_service_accounts
    args  = [self.input.instance_id.value]
  }
  ...
  container {
    ...
    graph {
      ...
      node {
        ...
        args = {
          compute_disk_ids = with.compute_instance_to_compute_disks.rows[*].disk_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_firewall_ids = with.compute_instance_to_compute_firewalls.rows[*].firewall_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_instance_group_ids = with.compute_instance_from_compute_instance_groups.rows[*].group_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_network_ids = with.compute_instance_to_compute_networks.rows[*].network_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_subnetwork_ids = with.compute_instance_to_compute_subnets.rows[*].subnetwork_id
        }
      }
      ...
      node {
        ...
        args = {
          iam_service_account_names = with.compute_instance_to_iam_service_accounts.rows[*].account_name
        }
      }
...
query "compute_instance_from_compute_instance_groups" {
  sql = <<-EOQ
    select
...
query "compute_instance_to_compute_disks" {
  sql = <<-EOQ
    select
...
query "compute_instance_to_compute_firewalls" {
  sql = <<-EOQ
    select
...
query "compute_instance_to_compute_networks" {
  sql = <<-EOQ
    select
...
query "compute_instance_to_compute_subnets" {
  sql = <<-EOQ
    select
...
query "compute_instance_to_iam_service_accounts" {
  sql = <<-EOQ
    select
