dashboard "gcp_compute_instance_age_report" {
  ...
  container {
    ...
    card {
      type  = "info"
      width = 2
      sql   = query.gcp_compute_instance_24_hours_count.sql
    }

    card {
      type  = "info"
      width = 2
      sql   = query.gcp_compute_instance_30_days_count.sql
    }

    card {
      type  = "info"
      width = 2
      sql   = query.gcp_compute_instance_30_90_days_count.sql
    }

    card {
      width = 2
      type  = "info"
      sql   = query.gcp_compute_instance_90_365_days_count.sql
    }

    card {
      width = 2
      type  = "info"
      sql   = query.gcp_compute_instance_1_year_count.sql
    }
  ...
  table {
    ...
    sql = query.gcp_compute_instance_age_table.sql
  }

}

query "gcp_compute_instance_24_hours_count" {
  sql   = <<-EOQ
    select
      count(*) as value,
      '< 24 hours' as label
...
query "gcp_compute_instance_30_days_count" {
  sql   = <<-EOQ
    select
      count(*) as value,
      '1-30 Days' as label
...
query "gcp_compute_instance_30_90_days_count" {
  sql   = <<-EOQ
    select
      count(*) as value,
      '30-90 Days' as label
...
query "gcp_compute_instance_90_365_days_count" {
  sql   = <<-EOQ
    select
      count(*) as value,
...
query "gcp_compute_instance_1_year_count" {
  sql   = <<-EOQ
    select
      count(*) as value,
      '> 1 Year' as label
...
query "gcp_compute_instance_age_table" {
  sql = <<-EOQ
    select
      i.name as "Name",
