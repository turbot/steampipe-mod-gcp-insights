dashboard "gcp_iam_role_dashboard" {
  ...
  container {
    ...
    card {
      query = query.gcp_iam_role_owner_count
      width = 2
    }
    ...
    card {
      query = query.gcp_iam_user_service_account_user_role_count
      width = 2
    }

    card {
      query = query.gcp_iam_user_service_account_token_creator_role_count
      width = 2
    }
...
query "gcp_iam_user_service_account_user_role_count" {
  sql = <<-EOQ
    ...
    from
      ...
      jsonb_array_elements_text(p -> 'members') as entity
    where
      p ->> 'role' = 'roles/iam.serviceAccountUser'
...
query "gcp_iam_user_service_account_token_creator_role_count" {
  sql = <<-EOQ
    with unapproved_bindings as (
      select
        project,
      ...
      from
        ...
        jsonb_array_elements_text(p -> 'members') as entity
      where
        p ->> 'role' = 'roles/iam.serviceAccountTokenCreator'
        and entity not like '%iam.gserviceaccount.com'
    )
    select
      count(b.entity) as value,
