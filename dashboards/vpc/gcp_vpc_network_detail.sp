dashboard "gcp_vpc_network_detail" {

  title         = "GCP VPC Network Detail"
  documentation = file("./dashboards/vpc/docs/gcp_vpc_network_detail.md")
  ...
  input "vpc_name" {
    ...
    query = query.gcp_vpc_network_input
    width = 4
  }
  ...
  container {
    ...
    card {
      width = 2
      query = query.gcp_vpc_network_subnet_count
      args = {
    ...
    card {
      width = 2
      query = query.gcp_vpc_network_is_default
      args = {
        name = self.input.vpc_name.value
      }
    }
    ...
    card {
      width = 2
      query = query.gcp_vpc_network_legacy
      args = {
        name = self.input.vpc_id.value
      }
    }
  ...
  container {
    ...
    container {
      ...
      table {
        title = "Tags"
        width = 6
        query = query.gcp_vpc_network_tags
        args = {
          name = self.input.vpc_name.value
        }
      }
    ...
    container {
      ...
      table {
        title = "CIDR Blocks"
        query = query.gcp_vpc_network_cidr_blocks
        args = {
      ...
      table {
        title = "DHCP Options"
        query = query.gcp_vpc_network_dhcp_options
        args = {
          vpc_id = self.input.vpc_id.value
        }
      }
  ...
  container {
    ...
    chart {
      ...
      type  = "column"
      width = 4
      query = query.gcp_vpc_network_subnet_by_az
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      query = query.gcp_vpc_network_subnets_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
  ...
  container {
    ...
    flow {
      query = query.gcp_vpc_network_routes_for_vpc_sankey
      args = {
        vpc_id = self.input.vpc_id.value
      }
    }
    ...
    table {
      title = "Route Tables"
      query = query.gcp_vpc_network_route_tables_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      title = "Routes"
      query = query.gcp_vpc_network_routes_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
  ...
  container {
    ...
    flow {
      title = "Peering Connections"
      width = 6
      query = query.gcp_vpc_network_peers_for_vpc_sankey
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      title = "Peering Connections"
      query = query.gcp_vpc_network_peers_for_vpc
      args = {
        vpc_id = self.input.vpc_id.value
      }
    }
  ...
  container {
    ...
    table {
      ...
      query = query.gcp_vpc_network_endpoints_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      title = "Gateways"
      query = query.gcp_vpc_network_gateways_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
...
query "gcp_vpc_network_subnet_count" {
  sql = <<-EOQ
    select
      'Subnets' as label,
      count(*) as value,
...
query "gcp_vpc_network_is_default" {
  sql = <<-EOQ
    select
...
query "gcp_vpc_network_num_ips_for_vpc" {
  sql = <<-EOQ
    with cidrs as (
      select
...
query "gcp_vpc_network_subnets_for_vpc" {
  sql = <<-EOQ
    with subnets as (
      select
        subnet_id,
...
query "gcp_vpc_network_security_groups_for_vpc" {
  sql = <<-EOQ
    select
    ...
    from
      gcp_vpc_network_security_group
    where
      vpc_id = $1
  EOQ
...
query "gcp_vpc_network_endpoints_for_vpc" {
  sql = <<-EOQ
    select
...
query "gcp_vpc_network_route_tables_for_vpc" {
  sql = <<-EOQ
    select
      route_table_id as "Route Table ID",
      tags ->> 'Name' as "Name"
    from
      gcp_vpc_network_route_table
    where
      vpc_id = $1
    order by
      route_table_id;
...
query "gcp_vpc_network_routes_for_vpc_sankey" {
  sql = <<-EOQ
    with routes as (
    select
        route_table_id,
      ...
      select
        ...
        'gcp_vpc_network_route_table' as category,
        0 as depth
      from
        routes
      union
...
query "gcp_vpc_network_routes_for_vpc" {
  sql = <<-EOQ
    select
      route_table_id as "Route Table ID",
      tags ->> 'Name' as "Name",
...
query "gcp_vpc_network_peers_for_vpc" {
  sql = <<-EOQ
    select
      id as "ID",
      tags ->> 'Name' as "Name",
    ...
    from
      gcp_vpc_network_peering_connection
    where
      requester_vpc_id = $1
...
query "gcp_vpc_network_gateways_for_vpc" {
  sql = <<-EOQ
    select
      internet_gateway_id as "ID",
      tags ->> 'Name' as "Name",
      'gcp_vpc_network_internet_gateway' as "Type",
      a ->> 'State' as "State"
    from
      gcp_vpc_network_internet_gateway,
    ...
    select
      id as "ID",
      tags ->> 'Name' as "Name",
      'gcp_vpc_network_egress_only_internet_gateway' as "Type",
      a ->> 'State' as "State"
    from
      gcp_vpc_network_egress_only_internet_gateway,
      jsonb_array_elements(attachments) as a
     where
      a ->> 'VpcId' = reverse(split_part(reverse($1), '/', 1))
    union all
    select
      ...
      tags ->> 'Name' as "Name",
      'gcp_vpc_network_vpn_gateway' as "Type",
      a ->> 'State' as "State"
    from
      gcp_vpc_network_vpn_gateway,
    ...
    select
      ...
      tags ->> 'Name' as "Name",
      'gcp_vpc_network_nat_gateway' as "Type",
      state as "State"
    from
      gcp_vpc_network_nat_gateway
     where
       vpc_id = $1
  EOQ
...
query "gcp_vpc_network_peers_for_vpc_sankey" {
  sql = <<-EOQ
    with peers as (
      select
        id,
...
query "gcp_vpc_network_overview" {
  sql = <<-EOQ
    select
      vpc_id as "VPC ID",
      title as "Title",
...
query "gcp_vpc_network_cidr_blocks" {
  sql = <<-EOQ
    select
...
query "gcp_vpc_network_dhcp_options" {
  sql = <<-EOQ
    select
    ...
    from
      gcp_vpc_network as v,
      gcp_vpc_network_dhcp_options as d
    where
      v.vpc_id = $1
...
query "gcp_vpc_network_subnet_by_az" {
  sql = <<-EOQ
    select
      availability_zone,
      count(*)
